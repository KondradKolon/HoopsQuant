...
async def get_upcoming_games(session: Session = Depends(get_db)):
    ...
    games = session.query(Game).filter(
        ...
        Game.home_score.is_(None)  # No score yet = upcoming
...
async def get_user_picks_stats(
    ...
    picks = session.query(UserPick).filter(
        ...
        UserPick.result != None  # Only completed picks
    ).all()
    ...
    return {
        ...
        "units_won": round(total_profit / 100, 2),  # Convert to "units" (1 unit = $100)
    }
...
async def get_leaderboard(
    ...
    return [{
        ...
        "roi": round((r[4] / (r[2] * 100)) * 100, 2) if r[2] else 0  # Assuming avg $100 bets
